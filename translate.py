    ...
    if uploaded_file is not None:
        ...
        with col2:
            ...
            if tools=='翻译工具':
                ...
                spraw="""逐段阅读这篇文献，总结核心内容，理解在这篇文献中，***YOUR prompt for message extract***。
                需要总结的关键点包括:
                ```TABLE header````，

                给出原文中对于以上信息的英文原句，必须是**原文完整句子**，
                - 句子必须：
                - 直接复制原文，**不可删减、截断或改写**；
                ...
                sp="""逐段阅读这篇文献，总结核心内容，理解在这篇文献中，***YOUR prompt for message extract***。
                需要总结的关键点包括:
                ```TABLE header````，
                ...
                更新信息时，遵循以下原则：
                对于来自同一个信息源的信息（如同为本文研究结果），优先级：使用具体的信息（如P值数值或方向）大于模糊的表述（未明确、不确定等文字表述）；
                对于来自不同信息源的信息（如本文研究结果、前人研究数据），应该保留所有信息，不要覆盖历史信息。
